...
test_multiobject_caustics() {
    echo "Multi-object caustics (two separated glass spheres, per-instance photon emission):"
    TIMEOUT=$CAUSTICS_TIMEOUT run_test "multi-object caustics" \
        --objects type=sphere:material=glass:pos=-1.5,0,0 \
        --objects type=sphere:material=glass:pos=1.5,0,0 \
...
test_area_light_caustics() {
    echo "Area-light soft caustics (disk emitter):"
    TIMEOUT=$CAUSTICS_TIMEOUT run_test "area light caustics" \
        --objects type=sphere:material=glass --caustics \
...
test_platonic_solids() {
    ...
    run_test "parametric-sphere-dsl" --scene examples.dsl.ParametricSphere --plane y:-2
