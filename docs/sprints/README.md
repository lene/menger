# Sprint Planning Documents

Active sprint planning documents for ongoing and future development work.

## Upcoming Sprints

| Sprint | Focus | Status | File |
|--------|-------|--------|------|
| 14 | Rendering Correctness & Code Health | In Progress | [SPRINT14.md](SPRINT14.md) |
| 15 | Visual Enhancements & Primitives | Planned | [SPRINT15.md](SPRINT15.md) |
| 16 | Developer Infrastructure & Website | Planned | [SPRINT16.md](SPRINT16.md) |
| 17 | Animation Tooling & DSL | Planned | [SPRINT17.md](SPRINT17.md) |
| 18 | Advanced Geometry | Planned | [SPRINT18.md](SPRINT18.md) |
| 19 | Materials, Textures & Backgrounds | Planned | [SPRINT19.md](SPRINT19.md) |
| 20 | GPU 4D Infrastructure | Planned | [SPRINT20.md](SPRINT20.md) |
| 21 | Higher-Dimensional Fractals | Planned | [SPRINT21.md](SPRINT21.md) |

## Completed Sprints

See [docs/archive/sprints/](../archive/sprints/) for completed sprint documentation:

| Sprint | Focus | Archived File |
|--------|-------|---------------|
| 13 | Visual Quality & Material Enhancements | [SPRINT13.md](../archive/sprints/SPRINT13.md) |
| 12 | t-Parameter Animation System | [SPRINT12.md](../archive/sprints/SPRINT12.md) |
| 11 | libGDX Wrapper, Thin-Film Physics, 4D Enhancements | [SPRINT11.md](../archive/sprints/SPRINT11.md) |
| 10 | Scala DSL for Scene Description | [SPRINT10.md](../archive/sprints/SPRINT10.md) |
| 9 | TesseractSponge (4D Sponges) | [SPRINT_9.md](../archive/sprints/SPRINT_9.md) |
| 8 | 4D Projection + UX Improvements | [SPRINT_8.md](../archive/sprints/SPRINT_8.md) |
| 7 | Materials & Textures | [SPRINT_7.md](../archive/sprints/SPRINT_7.md) |
| 6 | Full Geometry (IAS) | [SPRINT_6.md](../archive/sprints/SPRINT_6.md) |
| 5 | Triangle Mesh + Cube | [SPRINT_5.md](../archive/sprints/SPRINT_5.md) |

## Sprint Lifecycle

1. **Planning** - Sprint document created with goals, success criteria, and tasks
2. **Execution** - Document updated with progress, decisions, and issues
3. **Completion** - Document moved to archive with final notes
4. **Reference** - Archived document serves as historical record

## Document Format

Each sprint document typically includes:
- **Goals & Success Criteria** - What we're trying to achieve
- **Tasks & Estimates** - Detailed work breakdown
- **Technical Decisions** - Architecture and implementation choices
- **Testing Strategy** - How to verify success
- **References** - Related files and dependencies

## Related Documentation

- [ROADMAP.md](../../ROADMAP.md) - Strategic feature planning
- [TODO.md](../../TODO.md) - Quick notes and ideas
- [CODE_IMPROVEMENTS.md](../../CODE_IMPROVEMENTS.md) - Code quality assessments
- [optix-jni/ENHANCEMENT_PLAN.md](../../optix-jni/ENHANCEMENT_PLAN.md) - OptiX module roadmap
