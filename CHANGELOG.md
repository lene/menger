...
fixes ship together as one release.

### Added

- AI Scene Agent renderer-side support: `menger.tools.ManifestGenerator` (DSL capability
  manifest, CAP-7), `menger.tools.CorpusExporter` (example-scene corpus export), and
  `menger.tools.SceneValidator` (restricted-classpath compile + geometric-invariant checks,
  with an OS-level Docker sandbox in `docker/scene-validator/`).
- `--display` CLI option: re-execs as a child process with `DISPLAY` set explicitly, so the
  interactive window's display target is an explicit, injected parameter rather than ambient
  environment inheritance.
- `--render-lock-path` CLI option and an exclusive render-session lock: a second concurrent
  interactive-window launch is refused immediately instead of blocking or queuing.

### Changed

- **Breaking (scene files):** scene compilation now runs against a restricted classpath
  (`menger.dsl.RestrictedClasspath`) instead of the full JVM classpath. A `.scala` scene may
  reference the DSL surface and its transitive needs only — the Scala library, `menger-common`
  and `scala-logging`, plus `menger.dsl`, `menger.objects` and `menger.video`. Scenes that
  imported LibGDX, `io.github.lene.optix`, `upickle`, or menger's own `engines`/`tools`/`cli`/
  `input` packages no longer compile. This is deliberate (AD-4 rule 2): a generated scene is
  untrusted input, and the compile step is the boundary.

### Fixed

- `mengerApp`'s default `run`/`bgRun` main class is now pinned explicitly (`Main`) — sbt could
  no longer auto-detect a single main class once the tools above each added their own.
- `SceneValidator` reported every valid tesseract sponge at level ≥ 1 as `lint-findings`: the
  geometric invariants are properties of a *regular* 4D polytope, and 4D fractals were being
  run through them. Fractals now get the shape-independent subset only.
- `SceneValidator`'s geometric epsilons were absolute, so a legitimately small object
  (`Tesseract(size = 0.02f)`) was reported as entirely degenerate. They are now relative to
  the mesh's own scale.
- Logging went to stdout, interleaving timestamped log lines with the JSON payload
  `SceneValidator`, `ManifestGenerator` and `CorpusExporter` write there. Logback now targets
  stderr.
- `SceneValidator` emitted no JSON at all on a usage error, and crashed rather than reporting
  a tagged result on `StackOverflowError`/`OutOfMemoryError`/`LinkageError`. Both now produce
  AD-5 tagged output, and exit codes distinguish `refused` (2) from a scene defect (1).
- The `--display` re-exec dropped every JVM option the parent runs with, including
  `-Djava.library.path` — the child could not load the native library the window needs. The
  parent's options are now forwarded. `--display` combined with `--headless` no longer
  re-execs at all.
- `RenderLock` leaked a file descriptor when `tryLock()` failed with anything other than
  `OverlappingFileLockException`, and its default lock path was shared across all users of a
  machine (a second user got a misleading "failed to acquire" rather than "already held").
  The default path is now per-user.
- The scene-validator sandbox's wall-clock timeout killed the `docker` client rather than the
  container, so a hung scene kept its memory, CPUs and PID budget indefinitely. The container
  is now named and torn down explicitly.

### Security

- The scene-validator sandbox no longer passes `--gpus all`, mounts a writable `--output`
  directory, or mounts a `--textures` directory: none were used by the validator's pure-JVM
  code path, and together the last two reopened the copy-to-writable-destination path that
  the single-file scene mount was introduced to close.
- The vendored `seccomp-profile.json` has been removed. It was Docker's stock `default.json`
  verbatim (428 syscalls, `socket`/`connect`/`execve`/`ptrace` all permitted), so passing it
  to `--security-opt seccomp=` was identical in effect to passing nothing while reading as a
  control that existed. Containment is `--network none`, `--cap-drop ALL`,
  `no-new-privileges`, `--read-only` and the resource bounds.
- The sandbox image now pins its base image by digest and its JRE by version, and is labelled
  non-redistributable (it embeds a licensed OptiX SDK).
...
- Sprint 36 Phase B/C: manifest-driven `qa-runner.sh` replaces ad-hoc gate scripts —
  tiered gates (`push`/`pr`/`release`), `[i/N]` progress reporting, environment-aware
  skips (GPU busy, no CUDA) instead of false failures. `scripts/suites/smoke.sh` added
  as a fast pre-integration smoke tier.
- Sprint 36 Phase E/F: ratcheting gates against regressions — SonarCloud maintainability
  ratchet (F2), performance ratchet wrapping `benchmark.sh` (F4), sanitizer-warning-count
  ratchet in `memcheck.sh` (F3), shared `ratchet_check()` (fails closed on non-numeric
  input) in `lib.sh`.
- Sprint 36 Phase D: environment-aware CI gates — `test-full`/`coverage`/`run-smoke`
  jobs skip cleanly (not fail) when the shared GPU runner is busy, instead of reporting
  false failures.
- `docs/QA_INCIDENTS.md` — cross-repo QA incident log (Phase G3).

### Fixed

- `ConeSceneBuilder` now applies object rotation (was silently ignored) (H5.2).
- Global `--rot-x/-y/-z/-x-w/-y-w/-z-w` now applied to `--objects` CLI scenes (was a
  silent no-op) (H5.1).
- `color=` material property no longer silently dropped in certain object-spec paths
  (H5.3).
- L-system branch-drop and roll-axis bugs (H3.6).
- Video `--t` preview now time-maps correctly to a single still frame (H3.8).
- `--animate` no longer crashes without `--save-name` (H3.5).
- Klein bottle parametric surface seam from the `closedU` half-twist (H2.3).
- 4D polytopes now correctly scaled against the coordinate cross (H2.2).
- Planes now correctly occlude geometry behind them, instead of always rendering
  double-sided regardless of what's in front (H3.1).
- 4D edge endpoints are now clipped at the `eye_w` projection plane, fixing visual
  artifacts and console error spam near the projection boundary during interactive
  4D rotation.
- `LSystemSceneBuilder` now reports its real instance count instead of undercounting,
  fixing a silent instance-budget crash on complex L-systems.
- Ground plane restored across `manual-test.sh` scenes; fixed plane width regression.
- `TesseractEdgeSceneBuilder` no longer logs at `info` level on every scene rebuild
  (was spamming the console during interactive rotation) (Sprint 36 #2).
- `optix-jni` pin bumped `0.3.0` → `0.3.2`: fixes cone transparency, a depth-cutoff
  sign bug, triangle/sphere normals, and a bug where a second instance of the same
  GPU-4D-fast-path type (`menger4d`/`sierpinski4d`/`hexadecachoron4d`) was silently
  invisible — the custom-geometry SPI built one shared acceleration structure per
  geometry *type* instead of per instance, so the second instance's bounding volume
  never matched its actual position (Sprint 36 #12). New regression test
  `MengerRendererMultiInstanceSuite`; scripted two-instance coverage added to
  `integration-tests.sh`.

### Changed

- Sprint 36 Phase B3: `.git_hooks/pre-push` is now a thin wrapper over
  `qa-runner.sh --tier push`, replacing the earlier hand-maintained script.
- Sprint 36 A2/A4: retired the separate AI-review CI gate (superseded by the ratchets
  above); added `docs/ENFORCEMENT.md` mapping policy to enforcement mechanism.

## [0.8.11] - 2026-08-07

### Added

- Sprint 35 Phase 4 (Task 4.7, finding F14): `scripts/coverage-manifest.yaml` —
  single source of truth for cross-script rendering-feature coverage (13 materials,
  9 rendering flags). `ScriptParitySuite` extended to enforce bidirectional
  material/flag parity and manual⊆integration DSL-scene coverage. Nine manual-only
  DSL scenes and four materials ported to `integration-tests.sh`; `--fog` added to
  `manual-test.sh`. `ENFORCEMENT.md` rows #2/#3 (new-feature script coverage) now
  enforced (❌ → ✅).
- Sprint 35 Phase 4 (Task 4.6, finding F7): `FastPathRegressionSuite` now asserts
  the 4D fast path is actually taken when specs differ only in projection (update
  function called, proving zero `clearAllInstances`/rebuild), not just that 4D
...
- Sprint 35 Phase 4 (Task 4.4, finding F13): render-stats concern (per-frame
  ...
  JSON formatting now has its own test suite (`WithStatsSuite`).
- Sprint 35 Phase 4 (Task 4.5, finding F5): `benchmark.sh` now checks absolute
  P1 (5 s) / P2 (500 ms) ceilings alongside the ratio ratchet — a scene within
  1.3× baseline but over its absolute budget fails the gate.
...
- Sprint 35 Phase 4 (Task 4.9, CR-10): `rebuildScene` uses `Try.flatMap` + `fold`
  for a single declarative failure flow instead of nested `match`/`return`.
- Sprint 35 Phase 4 (Task 4.9, CR-12): `rgbIntsToColor` is now total (`take(4)`/
  `padTo`) instead of throwing `MatchError` on sub-4-component arrays;
  `EnvironmentConverters.parseColor` validates channel range.

## [0.8.10] - 2026-08-05

### Changed

- Sprint 35 Phase 4 (Task 4.3, finding F9): the three near-identical IFS 4D scene
  builders (`Menger4DSceneBuilder` / `Sierpinski4DSceneBuilder` /
  `Hexadecachoron4DSceneBuilder`) collapsed into one
  `Instanced4DSceneBuilder` parameterized by `IFS4DType`. The three identical engine
  fast-path state classes and `tryXxx4DFastPath` methods in `InteractiveEngine`
  collapsed into one `Instanced4DState` / `tryInstanced4DFastPath`, and the three
  `RotationFastPath` updaters into one `instanced4DUpdater` backed by a single
  `MengerRenderer.update4DProjection`. The three builder test suites consolidated
  into one parameterized `Instanced4DSceneBuilderSuite`.

## [0.8.9] - 2026-08-05

### Changed

- Sprint 35 Phase 4 (Task 4.2, finding F8): `InteractiveEngine`'s two hand-maintained
  4-way ORs for the 4D rotation fast-path now derive from a single
  `TypeRegistry.is4DFastPathType`, which reads the (previously write-only)
  `Entry.isProjected4D` flag. Fitness test: every `ObjectType.VALID_TYPES` entry
  resolves via `TypeRegistry.forType`. Regression test pins the new predicate against
  the old 4-way OR for every valid type.
...
- Pre-push change-detection now bases on `github/main` (post-migration the GitLab
  `origin/main` is stale, which fired the native gates on Scala-only pushes), and
  `.git/hooks/pre-push` dispatches to the pushing worktree's hook.

## [0.8.7] - 2026-08-03

### Changed

- Pinned optix-jni to the released **0.3.0** (Maven Central) — Sprint 35 Phase 3 (Release B):
  material now crosses the JNI seam as one packed `float[]` instead of positional floats (F2),
  `OptiXRenderer` is `AutoCloseable` with an atomic lifecycle handle swap (CR-6/CR-11), native
  runtime failures surface as the typed `OptiXException` with a catch-all on every `JNIEXPORT`
  (F11/CR-9, CR-4), and native logging is env-gated (F12) instead of raw `std::cerr`/`std::cout`.
  All changes are internal to optix-jni's private `@native` surface; menger's own code and public
  behavior are unaffected. Verified via the workspace's `compat.yml` (2416/2416 unit tests green
  against optix-jni@main before this pin bump).
...
[0.9.0]: https://github.com/lene/menger/compare/0.8.13...0.9.0
[0.8.13]: https://github.com/lene/menger/compare/0.8.12...0.8.13
[0.8.12]: https://github.com/lene/menger/compare/0.8.11...0.8.12
[0.8.11]: https://github.com/lene/menger/compare/0.8.10...0.8.11
[0.8.10]: https://github.com/lene/menger/compare/0.8.9...0.8.10
[0.8.9]: https://github.com/lene/menger/compare/0.8.8...0.8.9
[0.8.8]: https://github.com/lene/menger/compare/0.8.7...0.8.8
[0.8.7]: https://github.com/lene/menger/compare/0.8.6...0.8.7
[0.8.6]: https://github.com/lene/menger/compare/0.8.5...0.8.6
[0.8.5]: https://gitlab.com/lilacashes/menger/-/compare/0.8.4...0.8.5
