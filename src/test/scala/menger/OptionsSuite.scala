...
class OptionsSuite extends AnyFlatSpec with Matchers:
  ...
  class TestableMengerCLIOptions(args: Seq[String]) extends menger.MengerCLIOptions(args):
    override def onError(e: Throwable): Unit = throw e

  "empty options" should "give default timeout" in:
    val options = TestableMengerCLIOptions(Seq[String]())
    options.timeout() shouldEqual 0

  "--timeout" should "set timeout" in:
    val options = TestableMengerCLIOptions(Seq("--timeout", "1"))
  ...
  "--sponge-type" should "parse cube|square|tesseract|tesseract-sponge|tesseract-sponge-2" in:
    for spongeType <- Seq("cube", "square", "tesseract", "tesseract-sponge", "tesseract-sponge-2") do
      val options = TestableMengerCLIOptions(Seq("--sponge-type", spongeType))
  ...
  it should "throw ScallopException if invalid" in:
    an [ScallopException] should be thrownBy TestableMengerCLIOptions(Seq("--sponge-type", "invalid"))

  "--antialias-samples" should "set antialias samples" in:
    val options = TestableMengerCLIOptions(Seq("--antialias-samples", "1"))
    options.antialiasSamples() shouldEqual 1

  "--projection-screen-w" should "be valid with matching --projection-eye-w" in:
    val options = TestableMengerCLIOptions(Seq("--projection-screen-w", "1", "--projection-eye-w", "2"))
  ...
  it should "be invalid if <= 0" in:
    an [ScallopException] should be thrownBy
      TestableMengerCLIOptions(Seq("--projection-screen-w", "0", "--projection-eye-w", "2"))

  it should "be invalid if <= --projection-screen-w)" in:
    an [ScallopException] should be thrownBy
      TestableMengerCLIOptions(Seq("--projection-screen-w", "2", "--projection-eye-w", "2"))

  "rotation options" should "be valid if 0 <= x < 360" in:
    for opt <- Seq("--rot-x", "--rot-y", "--rot-z", "--rot-x-w", "--rot-y-w", "--rot-z-w") do
      val options = TestableMengerCLIOptions(Seq(opt, "1"))
      opt match
  ...
  it should "be invalid if >= 360" in:
    for opt <- Seq("--rot-x", "--rot-y", "--rot-z", "--rot-x-w", "--rot-y-w", "--rot-z-w") do
      an [ScallopException] should be thrownBy TestableMengerCLIOptions(Seq(opt, "360"))

  "--animate" should "default to empty animation specifications" in:
    TestableMengerCLIOptions(Seq()).animate.toOption shouldBe None

  it should "be invalid for bad AnimationSpecifications syntax" in:
    an [ScallopException] should be thrownBy
      TestableMengerCLIOptions(Seq("--animate", "0:1:2:3"))
  ...
  AnimationSpecification.TIMESCALE_PARAMETERS.foreach { timescale =>
    it should s"fail for a missing animated parameter when $timescale is specified" in:
      an[ScallopException] should be thrownBy
        TestableMengerCLIOptions(Seq("--animate", s"$timescale=10"))
  }
  
  it should "fail if both frames and seconds are specified" in:
    an[IllegalArgumentException] should be thrownBy
      TestableMengerCLIOptions(Seq("--animate", "frames=10:seconds=10"))
    
  it should "fail if no timescale is specified" in:
    an[ScallopException] should be thrownBy
      TestableMengerCLIOptions(Seq("--animate", "rot-x=0-10"))
  ...
  AnimationSpecification.TIMESCALE_PARAMETERS.foreach { timescale =>
    AnimationSpecification.ALWAYS_VALID_PARAMETERS.foreach { parameter =>
      it should s"succeed when $timescale and $parameter are specified" in:
        TestableMengerCLIOptions(Seq("--animate", s"$timescale=10:$parameter=0-10"))
    }
  }

  it should "fail if an invalid parameter is specified" in:
    an[ScallopException] should be thrownBy
      TestableMengerCLIOptions(Seq("--animate", "frames=10:invalid=0-10"))

  Seq("square", "cube").foreach { sponge =>
    it should s"fail if a 4D parameter is specified for 3D sponge type $sponge" in :
      an[ScallopException] should be thrownBy
        TestableMengerCLIOptions(Seq("--sponge-type", sponge, "--animate", "frames=10:rot-x-w=0-10"))
  }
  ...
  Seq("tesseract").foreach { sponge =>
    it should s"fail when level is specified for non-fractal sponge type $sponge" in:
      an[ScallopException] should be thrownBy
        TestableMengerCLIOptions(Seq("--sponge-type", sponge, "--animate", s"frames=10:level=0-1"))
  }
  ...
  it should "succeed when two valid animation specifications are given" in:
    TestableMengerCLIOptions(Seq("--animate", "frames=10:rot-x=0-10", "--animate", "frames=10:rot-x=10-20"))

  it should "return the correct animation parameters"  in:
    val options = TestableMengerCLIOptions(Seq("--animate", "frames=10:rot-x=0-10:rot-y=0-10"))
  ...
  it should "return correct animation parameters when two valid animation specifications are given" in:
    val options = TestableMengerCLIOptions(Seq("--animate", "frames=10:rot-x=0-10", "--animate", "frames=10:rot-x=10-20"))
  ...
  it should "fail if both frames and seconds are specified in separate specifications" in:
    an[ScallopException] should be thrownBy
      TestableMengerCLIOptions(Seq("--animate", "frames=10:rot-x=0-10", "--animate", "seconds=10:rot-x=10-20"))

  it should "fail if the same rotation is declared as static and animated" in:
    an[ScallopException] should be thrownBy
      TestableMengerCLIOptions(Seq("--animate", "frames=10:rot-x=0-10", "--rot-x", "10"))
