...
class FractionalLevelObjectSuite extends AnyFlatSpec with Matchers:

  class DummyFractionalLevelObject(val level: Float, val material: Material) extends FractionalLevelObject
  ...
  "calculateAlphaForFractionalLevel with original alpha = 1.0" should "return 1.0 when fractional part is 0" in:
    ...
    val alpha = DummyFractionalLevelObject(1.0, material).calculateAlpha()
    alpha shouldBe 1.0f

  it should "return 0.5 when fractional part is 0.5" in:
    val material = new Material(ColorAttribute.createDiffuse(new Color(1f, 1f, 1f, 1f)))
    val alpha = DummyFractionalLevelObject(1.5, material).calculateAlpha()
    alpha shouldBe 0.5f +- 0.001f

  it should "return ~0 when fractional part approaches 1" in:
    val material = new Material(ColorAttribute.createDiffuse(new Color(1f, 1f, 1f, 1f)))
    val alpha = DummyFractionalLevelObject(1.99, material).calculateAlpha()
