...
class Face4DSuite extends AnyFlatSpec with RectMesh with Matchers:
  ...
  private val seqXY = Seq(Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](1, -1, 0, 0), Vector[4, Float](1, 1, 0, 0), Vector[4, Float](-1, 1, 0, 0))
  private val faceXY = Face4D(seqXY)
  private val faceXZ = Face4D(Vector[4, Float](-1, 0, -1, 0), Vector[4, Float](1, 0, -1, 0), Vector[4, Float](1, 0, 1, 0), Vector[4, Float](-1, 0, 1, 0))
  private val faceXW = Face4D(Vector[4, Float](-1, 0, 0, -1), Vector[4, Float](1, 0, 0, -1), Vector[4, Float](1, 0, 0, 1), Vector[4, Float](-1, 0, 0, 1))
  private val faceYZ = Face4D(Vector[4, Float](0, -1, -1, 0), Vector[4, Float](0, 1, -1, 0), Vector[4, Float](0, 1, 1, 0), Vector[4, Float](0, -1, 1, 0))
  private val faceYW = Face4D(Vector[4, Float](0, -1, 0, -1), Vector[4, Float](0, 1, 0, -1), Vector[4, Float](0, 1, 0, 1), Vector[4, Float](0, -1, 0, 1))
  private val faceZW = Face4D(Vector[4, Float](0, 0, -1, -1), Vector[4, Float](0, 0, 1, -1), Vector[4, Float](0, 0, 1, 1), Vector[4, Float](0, 0, -1, 1))
  ...
  "a Face4D" should "convert to tuple and sequence correctly" in :
    faceXY.asTuple shouldBe(Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](1, -1, 0, 0), Vector[4, Float](1, 1, 0, 0), Vector[4, Float](-1, 1, 0, 0))
    faceXY.asSeq shouldBe Seq(Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](1, -1, 0, 0), Vector[4, Float](1, 1, 0, 0), Vector[4, Float](-1, 1, 0, 0))

  it should "translate correctly when adding a vector" in :
    val translated = faceXY + Vector[4, Float](2, 3, 4, 5)
    translated shouldBe Face4D(Vector[4, Float](1, 2, 4, 5), Vector[4, Float](3, 2, 4, 5), Vector[4, Float](3, 4, 4, 5), Vector[4, Float](1, 4, 4, 5))

  it should "scale correctly when dividing by a scalar" in :
    val face = Face4D(Vector[4, Float](-2, -2, 0, 0), Vector[4, Float](2, -2, 0, 0), Vector[4, Float](2, 2, 0, 0), Vector[4, Float](-2, 2, 0, 0))
    val scaled = face / 2.0f
    scaled shouldBe faceXY

  it should "test equality correctly" in :
    val face2 = Face4D(Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](1, -1, 0, 0), Vector[4, Float](1, 1, 0, 0), Vector[4, Float](-1, 1, 0, 0))
    val face3 = Face4D(Vector[4, Float](-2, -2, 0, 0), Vector[4, Float](2, -2, 0, 0), Vector[4, Float](2, 2, 0, 0), Vector[4, Float](-2, 2, 0, 0))
  ...
  "setIndices" should "return nothing with null vector" in:
    setIndices(Vector.Zero[4, Float]) shouldBe empty
  ...
  Seq(Vector.X, Vector.Y, Vector.Z, Vector.W).combinations(2).foreach {
    case List[Vector[4, Float]](v1, v2) =>
  ...
  Seq(Vector.X, Vector.Y, Vector.Z, Vector.W).combinations(3).foreach {
    case List[Vector[4, Float]](v1, v2, v3) =>
  ...
  "instantiating a Face4D from its vertices" should "create the normals" in:
    faceXY.normals.head should not be Vector.Zero[4, Float]
    faceXY.normals.last should not be Vector.Zero[4, Float]

  it should "create a face from a sequence of vectors" in:
    val vectors = Seq(Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](1, -1, 0, 0), Vector[4, Float](1, 1, 0, 0), Vector[4, Float](-1, 1, 0, 0))
    val face = Face4D(vectors)
    face shouldBe Face4D(Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](1, -1, 0, 0), Vector[4, Float](1, 1, 0, 0), Vector[4, Float](-1, 1, 0, 0))

  it should "throw exception when given less than 4 points" in:
    val vectors = Seq(Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](1, -1, 0, 0), Vector[4, Float](1, 1, 0, 0))
  ...
  it should "throw exception when vertices are not all same distance apart" in:
    val unevenVertices = Seq(Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](2, -1, 0, 0), Vector[4, Float](2, 1, 0, 0), Vector[4, Float](-1, 1, 0, 0))
  ...
  it should "throw exception when edges are not parallel to axes" in:
    val invalidPoints = Seq(Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](1, -0.9f, 0, 0), Vector[4, Float](1, 1, 0, 0), Vector[4, Float](-1, 1, 0, 0))
  ...
  it should "throw exception when edges are not orthogonal" in:
    val invalidPoints = Seq(Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](1, -1, 0, 0), Vector[4, Float](0, 1, 0, 0), Vector[4, Float](-1, 1, 0, 0))
    an [IllegalArgumentException] should be thrownBy Face4D(invalidPoints)

  "signs of the normals" should "be +/+ when starting in the positive sense in both face edge directions" in:
    val firstEdges = Seq(Vector[4, Float](1, 0, 0, 0), Vector[4, Float](0, 1, 0, 0))
  ...
  it should "also work in other planes than xy" in:
    Map(
      Seq(Vector[4, Float](1, 0, 0, 0), Vector[4, Float](0, 1, 0, 0)) -> Seq(1.0, 1.0),
      Seq(Vector[4, Float](-1, 0, 0, 0), Vector[4, Float](0, 1, 0, 0)) -> Seq(-1.0, 1.0),
      Seq(Vector[4, Float](1, 0, 0, 0), Vector[4, Float](0, -1, 0, 0)) -> Seq(1.0, -1.0),
      Seq(Vector[4, Float](-1, 0, 0, 0), Vector[4, Float](0, -1, 0, 0)) -> Seq(-1.0, -1.0),
      Seq(Vector[4, Float](1, 0, 0, 0), Vector[4, Float](0, 0, 1, 0)) -> Seq(1.0, 1.0),
      Seq(Vector[4, Float](-1, 0, 0, 0), Vector[4, Float](0, 0, 1, 0)) -> Seq(-1.0, 1.0),
      Seq(Vector[4, Float](1, 0, 0, 0), Vector[4, Float](0, 0, -1, 0)) -> Seq(1.0, -1.0),
      Seq(Vector[4, Float](-1, 0, 0, 0), Vector[4, Float](0, 0, -1, 0)) -> Seq(-1.0, -1.0),
      Seq(Vector[4, Float](1, 0, 0, 0), Vector[4, Float](0, 0, 0, 1)) -> Seq(1.0, 1.0),
      Seq(Vector[4, Float](-1, 0, 0, 0), Vector[4, Float](0, 0, 0, 1)) -> Seq(-1.0, 1.0),
      Seq(Vector[4, Float](1, 0, 0, 0), Vector[4, Float](0, 0, 0, -1)) -> Seq(1.0, -1.0),
      Seq(Vector[4, Float](-1, 0, 0, 0), Vector[4, Float](0, 0, 0, -1)) -> Seq(-1.0, -1.0),
      Seq(Vector[4, Float](0, 1, 0, 0), Vector[4, Float](0, 0, 1, 0)) -> Seq(1.0, 1.0),
      Seq(Vector[4, Float](0, -1, 0, 0), Vector[4, Float](0, 0, 1, 0)) -> Seq(-1.0, 1.0),
      Seq(Vector[4, Float](0, 1, 0, 0), Vector[4, Float](0, 0, -1, 0)) -> Seq(1.0, -1.0),
      Seq(Vector[4, Float](0, -1, 0, 0), Vector[4, Float](0, 0, -1, 0)) -> Seq(-1.0, -1.0),
      Seq(Vector[4, Float](0, 1, 0, 0), Vector[4, Float](0, 0, 0, 1)) -> Seq(1.0, 1.0),
      Seq(Vector[4, Float](0, -1, 0, 0), Vector[4, Float](0, 0, 0, 1)) -> Seq(-1.0, 1.0),
      Seq(Vector[4, Float](0, 1, 0, 0), Vector[4, Float](0, 0, 0, -1)) -> Seq(1.0, -1.0),
      Seq(Vector[4, Float](0, -1, 0, 0), Vector[4, Float](0, 0, 0, -1)) -> Seq(-1.0, -1.0),
      Seq(Vector[4, Float](0, 0, 1, 0), Vector[4, Float](0, 0, 0, 1)) -> Seq(1.0, 1.0),
      Seq(Vector[4, Float](0, 0, -1, 0), Vector[4, Float](0, 0, 0, 1)) -> Seq(-1.0, 1.0),
      Seq(Vector[4, Float](0, 0, 1, 0), Vector[4, Float](0, 0, 0, -1)) -> Seq(1.0, -1.0),
      Seq(Vector[4, Float](0, 0, -1, 0), Vector[4, Float](0, 0, 0, -1)) -> Seq(-1.0, -1.0)
  ...
  it should "create the correct normals in -xy" in:
    val face = Face4D(Vector[4, Float](1, -1, 0, 0), Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](-1, 1, 0, 0), Vector[4, Float](1, 1, 0, 0))
  ...
  "taking two consecutive corners out of a face" should "return the other corners in the correct order" in:
    val cornersToRemove = Seq(Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](1, -1, 0, 0))
    val remaining = remainingCorners(seqXY, cornersToRemove)
    remaining should contain only (Vector[4, Float](1, 1, 0, 0), Vector[4, Float](-1, 1, 0, 0))
  ...
  it should "fail if the number of corners to remove is wrong" in:
    val cornersToRemove = Seq(Vector[4, Float](-1, 1, 0, 0))
  ...
  it should "fail if any of the corners to remove is not part of the face" in:
    val cornersToRemove = Seq(Vector[4, Float](-2, -2, 0, 0), Vector[4, Float](1, -1, 0, 0))
    an [IllegalArgumentException] should be thrownBy remainingCorners(seqXY, cornersToRemove)

  "rotate around an edge" should "return a Face4D at all" in:
    val rotated = faceXY.rotate(Edge(Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](1, -1, 0, 0)))
  ...
  it should "rotate a selected Face4D correctly" in:
    val face = faceXY + Vector[4, Float](0, 0, -3, -3)
    face.normals should contain (Vector.Z)
    val rotated = face.rotate(Edge(Vector[4, Float](-1, -1, -3, -3), Vector[4, Float](1, -1, -3, -3)))
    rotated should contain (Face4D(Vector[4, Float](-1, -1, -3, -3), Vector[4, Float](1, -1, -3, -3), Vector[4, Float](1, -1, -1, -3), Vector[4, Float](-1, -1, -1, -3)))

  it should "rotate another selected Face4D correctly" in:
    val face = Face4D(Vector[4, Float](1, 1, 3, 3), Vector[4, Float](-1, 1, 3, 3), Vector[4, Float](-1, -1, 3, 3), Vector[4, Float](1, -1, 3, 3))
    face.normals should contain (-Vector.Z)
    val rotated = face.rotate(Edge(Vector[4, Float](1, 1, 3, 3), Vector[4, Float](-1, 1, 3, 3)))
    rotated should contain (Face4D(Vector[4, Float](1, 1, 3, 3), Vector[4, Float](-1, 1, 3, 3), Vector[4, Float](-1, 1, 1, 3), Vector[4, Float](1, 1, 1, 3)))
  ...
  it should "throw exception when corners are not in the face" in:
    val face = Face4D(Vector[4, Float](-1, -1, 0, 0), Vector[4, Float](1, -1, 0, 0), Vector[4, Float](1, 1, 0, 0), Vector[4, Float](-1, 1, 0, 0))
    an [IllegalArgumentException] should be thrownBy face.rotate(Edge(Vector[4, Float](-2, -2, 0, 0), Vector[4, Float](2, -2, 0, 0)))
