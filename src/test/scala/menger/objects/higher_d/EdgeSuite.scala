...
class EdgeSuite extends AnyFlatSpec with Matchers:
  "An Edge" should "be able to be created with 2 Vector4s" in:
    Edge(Vector4(0, 0, 0, 0), Vector4(1, 0, 0, 0))

  it should "have a dimension of 2" in:
    Edge(Vector4(0, 0, 0, 0), Vector4(1, 0, 0, 0)).dimension should be(2)

  it should "not be able to be created with 3 Vector4s" in:
    "Edge(Vector4(0, 0, 0, 0), Vector4(1, 0, 0, 0), Vector4(1, 1, 0, 0))" shouldNot typeCheck

  it should "not be able to be created with 1 Vector4" in:
    "Edge(Vector4(0, 0, 0, 0))" shouldNot typeCheck

  it should "correctly return the vertices" in:
    Edge(Vector4(0, 0, 0, 0), Vector4(1, 0, 0, 0)).values should be(Seq(Vector4(0, 0, 0, 0), Vector4(1, 0, 0, 0)))

  it should "correctly return the vertices by index" in:
    val edge = Edge(Vector4(0, 0, 0, 0), Vector4(1, 0, 0, 0))
    edge(0) should be(Vector4(0, 0, 0, 0))
    edge(1) should be(Vector4(1, 0, 0, 0))
  ...
  it should "calculate the diff between the vertices" in:
    Edge(Vector4(0, 0, 0, 0), Vector4(1, 0, 0, 0)).diff should be (Vector4(1, 0, 0, 0))

  it should "respect the order of the edges when calculating the diff" in:
    Edge(Vector4(1, 0, 0, 0), Vector4(0, 0, 0, 0)).diff should be (Vector4(-1, 0, 0, 0))

  "an Edge's string representation" should "be correct" in :
    val edge = Edge(Vector4(0, 0, 0, 0), Vector4(1, 1, 1, 1))
