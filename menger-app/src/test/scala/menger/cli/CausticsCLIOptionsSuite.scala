...
class CausticsCLIOptionsSuite extends AnyFlatSpec with Matchers:
  ...
  "--caustics" should "default to false when not provided" in:
    val opts = SafeMengerCLIOptions(Seq("--objects", "type=sphere"))
  ...
  it should "accept custom photon count" in:
    val opts = SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-photons", "50000"))
    opts.causticsPhotons() shouldBe 50000

  it should "accept maximum photon count (10 million)" in:
    val opts = SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-photons", "10000000"))
  ...
  it should "reject photon count exceeding 10 million" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-photons", "10000001"))

  it should "reject zero photon count" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-photons", "0"))

  it should "reject negative photon count" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-photons", "-1"))

  it should "require --caustics flag" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics-photons", "50000"))
  ...
  it should "accept custom iteration count" in:
    val opts = SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-iterations", "50"))
    opts.causticsIterations() shouldBe 50

  it should "accept maximum iteration count (1000)" in:
    val opts = SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-iterations", "1000"))
    opts.causticsIterations() shouldBe 1000

  it should "reject iteration count exceeding 1000" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-iterations", "1001"))

  it should "reject zero iterations" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-iterations", "0"))
  ...
  it should "accept maximum radius (10.0)" in:
    val opts = SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-radius", "10.0"))
    opts.causticsRadius() shouldBe 10.0f

  it should "reject radius exceeding 10.0" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-radius", "10.1"))

  it should "reject zero radius" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-radius", "0.0"))

  it should "reject negative radius" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-radius", "-0.5"))

  it should "require --caustics flag" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics-radius", "0.2"))
  ...
  it should "accept custom alpha value" in:
    val opts = SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-alpha", "0.5"))
    opts.causticsAlpha() shouldBe 0.5f

  it should "reject alpha >= 1.0" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-alpha", "1.0"))

  it should "reject alpha <= 0.0" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-alpha", "0.0"))

  it should "reject negative alpha" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics", "--caustics-alpha", "-0.5"))

  it should "require --caustics flag" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--objects", "type=sphere", "--caustics-alpha", "0.8"))

  "all caustics options" should "work together" in:
    val opts = SafeMengerCLIOptions(Seq(
      "--objects", "type=sphere", "--caustics",
