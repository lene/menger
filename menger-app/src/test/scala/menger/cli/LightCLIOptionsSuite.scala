...
class LightCLIOptionsSuite extends AnyFlatSpec with Matchers:
  ...
  it should "parse a single directional light with position only" in:
    val opts = SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "directional:1.0,2.0,3.0"))
  ...
  it should "parse a single point light with position only" in:
    val opts = SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "point:0.0,-5.0,10.0"))
    opts.light.toOption shouldBe defined
    val lights = opts.light()
    lights should have size 1
  ...
  it should "parse a directional light with intensity" in:
    val opts = SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "directional:-1.0,1.0,-1.0:2.5"))
    val lights = opts.light()
    lights should have size 1
    lights.head.lightType shouldBe LightType.DIRECTIONAL
  ...
  it should "parse a point light with intensity" in:
    val opts = SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "point:0.0,5.0,0.0:0.5"))
    val lights = opts.light()
    lights should have size 1
    lights.head.lightType shouldBe LightType.POINT
  ...
  it should "parse a light with hex color (no intensity)" in:
    val opts = SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "directional:1.0,0.0,0.0::ff0000"))
    val lights = opts.light()
    lights should have size 1
    lights.head.lightType shouldBe LightType.DIRECTIONAL
    lights.head.intensity shouldEqual 1.0f
  ...
  it should "parse a light with intensity and hex color" in:
    val opts = SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "point:0.0,5.0,0.0:2.0:00ff00"))
    val lights = opts.light()
    lights should have size 1
    lights.head.lightType shouldBe LightType.POINT
  ...
  it should "parse a light with intensity and RGB color" in:
    val opts = SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "directional:0.0,1.0,0.0:1.5:255,0,0"))
    val lights = opts.light()
    lights should have size 1
  ...
  it should "parse up to 8 lights" in:
    val lightArgs = Seq("--optix", "--objects", "type=sphere") ++
  ...
  it should "reject invalid light type" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "invalid:1.0,0.0,0.0"))

  it should "reject malformed light specification (missing position)" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "directional:"))

  it should "reject malformed light specification (incomplete position)" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "directional:1.0,2.0"))

  it should "reject malformed light specification (non-numeric position)" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "directional:x,y,z"))

  it should "reject malformed light specification (non-numeric intensity)" in:
    an[ScallopException] should be thrownBy
      SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "directional:1.0,0.0,0.0:bright"))
  ...
  it should "accept negative intensity values" in:
    val opts = SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "directional:1.0,0.0,0.0:-0.5"))
    val lights = opts.light()
    lights.head.intensity shouldEqual -0.5f

  it should "accept zero intensity" in:
    val opts = SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "point:0.0,5.0,0.0:0.0"))
    val lights = opts.light()
    lights.head.intensity shouldEqual 0.0f

  it should "parse case-insensitive light types" in:
    val opts1 = SafeMengerCLIOptions(Seq("--optix", "--objects", "type=sphere", "--light", "DIRECTIONAL:1.0,0.0,0.0"))
