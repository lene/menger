...
class TriangleMeshDataSuite extends AnyFlatSpec with Matchers:
  ...
  // Use legacy 6-float stride for basic validation tests
  private val stride6 = TriangleMeshData.LegacyVertexStride

  "TriangleMeshData" should "accept valid vertex and index data" in:
    val vertices = Array(0f, 0f, 0f, 0f, 0f, 1f)
    val indices = Array(0, 0, 0)
    val mesh = TriangleMeshData(vertices, indices, stride6)
    mesh.numVertices should be(1)
    mesh.numTriangles should be(1)

  it should "reject vertices not divisible by stride" in:
    val vertices = Array(0f, 0f, 0f, 0f, 0f)
    an[IllegalArgumentException] should be thrownBy:
      TriangleMeshData(vertices, Array.emptyIntArray, stride6)

  it should "reject indices not divisible by 3" in:
    an[IllegalArgumentException] should be thrownBy:
      TriangleMeshData(Array.emptyFloatArray, Array(0, 1), stride6)

  it should "accept empty arrays" in:
    val mesh = TriangleMeshData(Array.emptyFloatArray, Array.emptyIntArray, stride6)
  ...
  "Face.toTriangleMesh" should "produce 4 vertices and 2 triangles" in:
    forAll(Seq(X, Y, Z, -X, -Y, -Z)) { normal =>
      ...
      mesh.vertexStride should be(8) // Now includes UVs
    }
  ...
  it should "have vertices at correct positions for Z normal" in:
    ...
    val positions = (0 until 4).map { i =>
      (mesh.vertices(i * stride), mesh.vertices(i * stride + 1), mesh.vertices(i * stride + 2))
    }
  ...
  it should "have correct normals for Z normal" in:
    ...
    val stride = mesh.vertexStride
    for i <- 0 until 4 do
      mesh.vertices(i * stride + 3) should be(0f)
      mesh.vertices(i * stride + 4) should be(0f)
      mesh.vertices(i * stride + 5) should be(1f)
  ...
  it should "include UV coordinates" in:
    val face = Face(0, 0, 0, 1, Z)
    val mesh = face.toTriangleMesh
    mesh.hasUVs should be(true)
    val stride = mesh.vertexStride
    val uvs = (0 until 4).map { i =>
      (mesh.vertices(i * stride + 6), mesh.vertices(i * stride + 7))
    }
    uvs should contain((0f, 0f))
    uvs should contain((1f, 0f))
    uvs should contain((1f, 1f))
    uvs should contain((0f, 1f))
