...
class PentachoronMeshSuite extends AnyFlatSpec with Matchers:
  ...
  private def mesh(
    ...
    Mesh4DProjection(Pentachoron(size), center, eyeW, screenW, rotXW, rotYW, rotZW)

  "PentachoronMesh" should "generate correct vertex and triangle counts" in:
    val data = mesh().toTriangleMesh
    // 10 triangular faces x 3 vertices = 30 vertices
    data.numVertices shouldBe 30
    // 10 faces x 1 triangle each = 10 triangles
  ...
  it should "produce different projections for different eye distances" in:
    // Pentachoron has most vertices at negative w, so far-eye can project larger than near-eye.
    // Just verify eye distance changes the output.
    val near = mesh(eyeW = 3f,  screenW = 1.5f).toTriangleMesh
    val far  = mesh(eyeW = 10f, screenW = 5f).toTriangleMesh
    near.vertices should not equal far.vertices
  ...
  it should "have visible 4D structure with default rotation" in:
    ...
    m.toTriangleMesh.numTriangles shouldBe 10
