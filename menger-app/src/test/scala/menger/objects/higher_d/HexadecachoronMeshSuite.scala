...
class HexadecachoronMeshSuite extends AnyFlatSpec with Matchers:
  ...
  private def mesh(
    ...
    Mesh4DProjection(Hexadecachoron(size), center, eyeW, screenW, rotXW, rotYW, rotZW)

  "HexadecachoronMesh" should "generate correct vertex and triangle counts" in:
    val data = mesh().toTriangleMesh
    // 32 triangular faces x 3 vertices = 96 vertices
    data.numVertices shouldBe 96
    // 32 faces x 1 triangle each = 32 triangles
  ...
  it should "have visible 4D structure with default rotation" in:
    ...
    m.toTriangleMesh.numTriangles shouldBe 32
